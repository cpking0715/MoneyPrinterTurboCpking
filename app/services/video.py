...
def combine_videos(
    ...
    logger.info(f"minimum clip duration: {min_clip_duration} seconds")
    ...
    for video_path in video_paths:
        ...
        while start_time < clip_duration:
            ...
            # 保留所有有效分段，但过滤掉短于 min_clip_duration 的零头片段。
            # 这样既不会丢掉"整段视频本身就短于 max_clip_duration"的素材，
            # 也不会把长视频最后不足 1 秒的碎片塞进合成结果。
            seg_duration = end_time - start_time
            if seg_duration > 0 and seg_duration >= min_clip_duration:
